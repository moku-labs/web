...
| `<path>/index.html` (per route) | pages | always — EXCEPT a `spa`-mode client-only route (dynamic, no `.generate()`), which is skipped and rendered on the client |
